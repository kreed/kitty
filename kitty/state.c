...
kitty_chrome_frame(const OSWindow *os_window, unsigned *x, unsigned *y) {
    ...
    if (!global_state.is_wayland || !os_window->handle || OPT(hide_window_decorations) ||
        glfwGetWindowAttrib(os_window->handle, GLFW_MAXIMIZED) || is_os_window_fullscreen((OSWindow *)os_window))
        return false;
